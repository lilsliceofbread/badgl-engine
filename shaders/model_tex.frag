...
void main()
{
    ...
    vec3 ambient = texture(texture_diffuse, fs_in.uv).xyz * light.ambient;

    /* diffuse */

    vec3 light_dir = normalize(light_pos - frag_pos);
    float diffuse_strength = max(dot(normal, light_dir), 0.0);
    vec3 diffuse = (diffuse_strength * texture(texture_diffuse, fs_in.uv).xyz) * light.diffuse;
    ...
    vec3 specular = (specular_strength * texture(texture_specular, fs_in.uv).xyz) * light.specular;
